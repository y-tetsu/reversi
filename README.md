...
選択可能な対戦相手(AI戦略)の一覧です。難易度は8x8サイズの場合の目安となっております。CPUの持ち時間は0.5秒までとしています。
 ...
 |SlowStarter |残り手数が15%未満の場合はUnselfish戦略、15%以上の場合はGreedy戦略を取ります。 | ★ |
 |Table |評価テーブルにより盤面を評価して手を選びます。なるべく少なく取り、角を狙い、角のそばは避けます。 | ★ |
 |MinMax |ミニマックス法で2手先を読んで手を選びます。Table戦略に加えて、配置可能数と勝敗を盤面評価の考慮に入れます。 | ★★ |
