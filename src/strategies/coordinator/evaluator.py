...
class Evaluator_E(AbstractEvaluator):
    ...
    辺のパターンの評価値を算出
    """
    def __init__(self, wpy=50, wy=100, wpwin=-15, wwin=-30):
        self.scorer = EdgeScorer(wpy, wy, wpwin, wwin)  # 辺のパターンによる評価値算出
...
class Evaluator_TPWE(AbstractEvaluator):
    ...
    """
    def __init__(self, size=8, corner=50, c=-20, a1=0, a2=-1, b=-1, x=-25, o=-5, wp=5, ww=10000, wpy=50, wy=100, wpwin=-15, wwin=-30):
        self.t = Evaluator_T(size, corner, c, a1, a2, b, x, o)
        self.p = Evaluator_P(wp)
        self.w = Evaluator_W(ww)
        self.e = Evaluator_E(wpy, wy, wpwin, wwin)
